...
class NewFormer(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.transformer32 = Transformer(self.embed_dims[-1], self.depths[-1], 8, 256, self.embed_dims[-1] * 4)
        self.transformer16 = Transformer(self.embed_dims[-2], self.depths[-2], 8, 128, self.embed_dims[-2] * 4)
        self.transformer8 = Transformer(self.embed_dims[-3], self.depths[-3], 8, 64, self.embed_dims[-3] * 4)
        self.transformer4 = Transformer(self.embed_dims[-4], self.depths[-4], 8, 32, self.embed_dims[-4] * 4)
        ...
        self.decoder = Decoder(self.embed_dims, num_classes=2)
    ...
    def get_top_indices(self, original_indices, attn_map, current_patch_size, target_patch_size):
        ...
        cutoff = int(0.5 * attn_map.shape[1])
